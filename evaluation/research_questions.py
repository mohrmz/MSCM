"""
MSCM: Method Source Code Metrics Measurement

This module contains method to visualize the results of MSCM-1
...
def plot_MSCM_pefromance_changes():
    df = pd.read_excel(dir + 'evaluation_MSCM1.xlsx', index_col=False)
    df1 = df.melt(id_vars=['Model', 'Method filter'],
...
def plot_samples_histogram():
    ...
    df_name = 'MSCM_charts.xlsx'
    df = pd.read_excel(df_path + df_name,
                       sheet_name='manual-evaluation-samples')
    ...
    df2 = pd.melt(df,
                  # id_vars=['Project ID'],
                  var_name=['Test set'],
                  value_vars=['All methods',
                              'Methods selected for manual evaluation'],
                  value_name='Number of methods',
                  )
